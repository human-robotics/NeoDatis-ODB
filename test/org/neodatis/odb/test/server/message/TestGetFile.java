 ...
 *
 */
public class TestGetFile extends ODBTest{
	public void test1() throws IOException{
		...
		NeoDatisConfig config = NeoDatis.getConfig().setInboxDirectory(DIRECTORY+"inbox").setBaseDirectory(DIRECTORY);
		...
		try{
			// Opens the client
			config.setHostAndPort("localhost", port);
			ODB odb = NeoDatis.openClient(baseName, config);

			// creates a simple file
			String s = "This is a test string!";
			String remoteFileName = DIRECTORY+ baseName+".txt";
			FileOutputStream fos = new FileOutputStream(remoteFileName);
			fos.write(s.getBytes());
			fos.close();
			// keep the size
			long size = new File(remoteFileName).length();
			
			// sends the file
			GetFileMessageResponse fmr = odb.ext().getFile(false,  remoteFileName, true, "f1.txt");
			odb.close();
			...
			File f = new File(DIRECTORY+"inbox/f1.txt");
			assertTrue(f.exists());
