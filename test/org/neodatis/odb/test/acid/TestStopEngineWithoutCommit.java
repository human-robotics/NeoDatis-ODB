...
public class TestStopEngineWithoutCommit extends ODBTest {
	...
	public void t1estK1() throws Exception {
		...
		odb.store(getInstance("f1"));
		odb.store(getInstance("f2"));
		ObjectOid oid = odb.store(getInstance("f3"));
	...
	public void t1estN2() throws Exception {
		ODB odb = test.open("acid1");
		if (simpleObject) {
			Objects objects = odb.query(Function.class, W.equal("name", "f1000")).objects();
			Function f = (Function) objects.first();
			f.setName("new name");
			odb.store(f);
		} else {
			Objects objects = odb.query(User.class, W.equal("name", "f1000")).objects();
			User f = (User) objects.first();
			f.setName("new name");
			odb.store(f);
		}
		odb.commit();
	...
	public void t1estN3() throws Exception {
		ODB odb = test.open("acid1");
		if (simpleObject) {
			Objects objects = odb.query(Function.class, W.equal("name", "new name")).objects();
			odb.delete(objects.first());
		} else {
			Objects objects = odb.query(User.class, W.equal("name", "new name")).objects();
			odb.delete(objects.first());
		}
		odb.commit();
	}
