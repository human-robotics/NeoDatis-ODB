...
public class TestSubList extends ODBTest {
	...
	public void test1() throws IOException, Exception {
		...
		odb = open(baseName);
		ValuesQuery q = odb.queryValues(Handler.class).field("parameters").sublist("parameters", "sub1", 1, 5, true).sublist(
	...
	public void test11() throws IOException, Exception {
		...
		odb = open(baseName);
		ValuesQuery q = odb.queryValues(Handler.class).field("parameters").sublist("parameters", "sub1", 1, 5, true).sublist("parameters", "sub2", 1, 10).size(
				"parameters", "size");
		Values values = q.values();
	...
	public void test2() throws IOException, Exception {
		...
		ObjectOid oid = odb.store(handler);
		odb.close();
		...
		Values values = odb.queryValues(Handler.class).sublist("parameters", "sub", 490, 5, true).size("parameters", "size").values();
	...
	public void test3() throws IOException, Exception {
		...
		ValuesQuery q = odb.queryValues(Handler.class).sublist("parameters", "sub", 9990, 5, true);
		q.setReturnInstance(false);
		Values values = q.values();
		long end = OdbTime.getCurrentTimeInMs();
		println("time to load sublist of 5 itens from 10000 : " + (end - start));
		println(values);
		ObjectValues ov = values.nextValues();
	...
	public void test5() throws IOException, Exception {
		...
		odb = open(baseName);
		long start = OdbTime.getCurrentTimeInMs();
		ValuesQuery q = odb.queryValues(Handler.class).sublist("parameters", "sub", 0, 2, true);
		Values values = q.values();
		long end = OdbTime.getCurrentTimeInMs();
	...
	public void test6() throws IOException, Exception {
		...
		odb = open(baseName);
		long start = OdbTime.getCurrentTimeInMs();
		Query q = odb.query(Handler.class);
		Objects objects = odb.getObjects(q);
		long end = OdbTime.getCurrentTimeInMs();

		Handler h = (Handler) objects.first();
	...
	public void test4() {
		...
		odb = open(baseName);
		User u = (User) odb.query(User.class).objects().first();

		ValuesQuery q = odb.queryValues(Profile.class).field("name").sublist("functions", 1, 2, false).size("functions", "fsize");
		q.orderByAsc("name");
		Values v = q.values();
		i = 0;
		while (v.hasNext()) {
