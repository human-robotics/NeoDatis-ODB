...
public class PerformanceTest1WithIndex extends ODBTest {
	...
	public void t1estInsertSimpleObjectODB(int size) throws Exception {
		...
		for (int j = 0; j < TEST_SIZE; j++) {
			...
			q = odb.query(SimpleObject.class, W.equal("name", "Bonjour, comment allez vous?" + j));
			Objects objects = odb.getObjects(q);
			assertTrue(q.getExecutionPlan().useIndex());
			so = (SimpleObject) objects.first();
		...
		if (doUpdate) {
			...
			so = null;
			q = odb.query(SimpleObject.class);
		...
		if (doDelete) {
			...
			while (l.hasNext()) {
				so = (SimpleObject) l.next();
				ObjectOid oid = odb.getObjectId(so);
				//println("before delete object with oid " + oid + " , " + so.getName());
