...
public class TestBigDecimal extends ODBTest {
	...
	public void test1(){
		...
		try{
			...
			odb = open(baseName);
			Objects<ItemVenda> itens = odb.getObjects(ItemVenda.class);
	...
	public void test12(){
		...
		try{
			...
			odb = open(baseName);
			Objects<ItemVenda> ii = odb.query(ItemVenda.class, W.equal("id", new Long(1))).objects(); 
			ItemVenda iv2 = ii.first(); 
			odb.delete(iv2);
			odb.close();
	...
	public void test13(){
		...
		try{
			...
			while(ii.hasNext()){
				...
				ItemVenda iv = (ItemVenda) odb.query(ItemVenda.class,W.equal("id", ivOriginal.getId())).objects().first();
				odb.delete(iv);
				odb.commit();
				odb.close();
			}
