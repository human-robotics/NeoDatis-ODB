...
public class TestSettingExistingObject extends ODBTest {
	...
	public void test2(){
		...
		try{
			...
			odb = open(baseName);
			Objects<Category> categories = odb.query(Category.class,W.equal("name", "Category 2")).objects();
			Category c2 = categories.first();
			
			// Get all itens of Category 1
			Objects<Item> itens = odb.query(Item.class,W.equal("category.name", "Category 1")).objects();
